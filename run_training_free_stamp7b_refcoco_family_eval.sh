...
if command -v flock >/dev/null 2>&1; then
  exec 9>"${ROOT}/outputs/.training_free_stamp7b_refcoco_family.lock"
  if ! flock -n 9; then
    echo "Another STAMP-7B RefCOCO-family Training-Free job already holds the lock." >&2
    exit 0
  fi
fi
...
echo "[2/3] Exporting/resuming STAMP-7B dumps and applying the frozen refiner"
SUMMARY_ARGS=()
...
echo "[3/3] Combining all split summaries"
python -m training_free_refine.summarize_splits \
  "${SUMMARY_ARGS[@]}" \
  --output-dir "${COMBINED_OUTPUT}" \
  --title "STAMP-7B Training-Free RefCOCO and RefCOCO+ Evaluation"

echo "STAMP-7B RefCOCO-family Training-Free evaluation completed."
