...
class PatchSelector(nn.Module):
    """Select token-grid patches that are useful for local refinement."""

    def __init__(self, top_k: int = 64, boundary_weight: float = 0.5, mode: str = "hybrid") -> None:
        super().__init__()
        if top_k <= 0:
    ...
    @torch.no_grad()
    def forward(self, mask_logits: torch.Tensor, grid_hw: tuple[int, int]) -> dict[str, torch.Tensor]:
        if mask_logits.ndim != 3 or mask_logits.shape[-1] != 2:
            raise ValueError(f"mask_logits must have shape [B, N, 2], got {tuple(mask_logits.shape)}")
        ...
        fg_prob = torch.softmax(mask_logits, dim=-1)[..., 1].reshape(batch_size, 1, grid_h, grid_w)
